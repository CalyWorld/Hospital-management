...
function seedDatabase() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            // SuperAdmin
            const superAdmin = yield superAdmin_1.SuperAdmin.create({
                username: "superadmin1",
                password: yield hashPassword("superadminpass"),
                createdAt: randomDateInRange(currentMonthStart, nextMonthEnd),
            });
            // Hospitals
            const hospitals = yield hospital_1.Hospital.insertMany([
                {
                    name: "City Hospital",
                    address: "123 Main St",
                    createdAt: randomDateInRange(currentMonthStart, nextMonthEnd),
            ...
            // Admins
            const admins = yield admin_1.Admin.insertMany([
                {
                    username: "admin1",
                    password: yield hashPassword("password1"),
                    role: "hospital-admin",
                ...
                {
                    username: "admin3",
                    password: yield hashPassword("password3"),
                    role: "hospital-admin",
                ...
                {
                    username: "admin4",
                    password: yield hashPassword("password4"),
                    role: "hospital-admin",
                ...
                {
                    username: "admin5",
                    password: yield hashPassword("password5"),
                    role: "hospital-admin",
                    hospital: hospitals[4]._id,
                    createdAt: randomDateInRange(currentMonthStart, nextMonthEnd),
                },
            ]);
            yield superAdmin_1.SuperAdmin.findByIdAndUpdate(superAdmin._id, {
            ...
            for (let i = 0; i < hospitals.length; i++) {
                for (let j = 0; j < 5; j++) {
                    ...
                    const availabilityStart = i === 0 && j === 0
                        ? todayAvailabilityStart
                        : i === 0 && j === 1
                            ? tomorrowAvailabilityStart
                            : i === 0 && j === 2
                                ? nextWeekAvailabilityStart
                                : atLocalTime(randomDateInRange(currentMonthStart, nextMonthEnd), 8 + (j % 3), 0);
            ...
            const doctors = yield doctor_1.Doctor.insertMany(doctorData);
            const patients = yield patient_1.Patient.insertMany(patientData);
            // Updating patients with their corresponding doctors
            for (const patient of patients) {
                const randomDoctor = doctors[Math.floor(Math.random() * doctors.length)];
                yield patient_1.Patient.findByIdAndUpdate(patient._id, {
                    doctor: [randomDoctor._id],
                });
                yield doctor_1.Doctor.findByIdAndUpdate(randomDoctor._id, {
                    $push: { patients: patient._id },
                });
            }
            // HealthRecords
            const healthRecords = yield healthRecords_1.HealthRecord.insertMany(patients.map((patient) => ({
                patient: patient._id,
                treatments: [], // Will add treatments separately
                createdAt: randomDateInRange(currentMonthStart, nextMonthEnd),
            })));
            // Medications
            const medications = yield medication_1.Medication.insertMany([
                {
                    name: "Aspirin",
                    quantity: 30,
                    fee: 10,
            ...
            // Treatments
            const treatments = yield treatment_1.Treatment.insertMany(patients.map((patient, index) => ({
            ...
            yield Promise.all(healthRecords.map((record, index) => healthRecords_1.HealthRecord.findByIdAndUpdate(record._id, {
                treatments: [treatments[index % treatments.length]].map((treatment) => treatment._id),
            })));
            // Appointments
            yield appointments_1.Appointment.insertMany(patients.map((patient, index) => {
                const chosenDoctor = doctors[Math.floor(Math.random() * doctors.length)];
                const anchorDate = index % 10 === 0
                    ? todayAvailabilityStart
                    : index % 10 === 1
                        ? tomorrowAvailabilityStart
                        : index % 10 === 2
            ...
            // Messages
            yield messages_1.Message.insertMany([
                {
